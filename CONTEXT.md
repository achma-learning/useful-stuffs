...
- `index.html` — **the whole app.** ~1770 lines: a `<style>` block (~263 lines), the content markup (20 sections), and a `<script>` block (~42 lines) at the bottom. This is the only file you ever need to edit for content.
...
- **Last shipped (2026-08-30, third change):** Added a new `#esc` section (between `#francais` and `#portails`, own nav pill) for the EU's European Solidarity Corps ("Youth Europa") — fully-funded volunteering for 17-30 year-olds, Morocco eligible as an EU-neighbouring third country, registration via EU Login on youth.europa.eu/solidarity, CSM Maroc named as a local Quality-Label-holding partner org. Also added an h3 sub-block to the existing `#casier` section for epolice.ma's شهادة حسن السيرة (certificate of good conduct) — an 8-step checklist plus a tip explicitly distinguishing it from the bulletin n°3 casier judiciaire already covered there (different issuer: DGSN/police vs. ministère de la Justice; epolice.ma launched Dec 2024). Both sourced via WebSearch/WebFetch (youth.europa.eu, chouf-alkhabar.com step-by-step guide, maroc.ma DGSN launch announcement) rather than filled from memory.
